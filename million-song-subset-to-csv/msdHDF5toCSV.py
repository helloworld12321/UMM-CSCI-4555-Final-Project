...
def main():
    ...
    #the user,
        ...
        csvRowString = ("SongID,TrackID,AlbumID,AlbumName,ArtistID,ArtistLatitude,ArtistLocation,"+
            "ArtistLongitude,ArtistName,Danceability,Duration,KeySignature,"+
    ...
    for root, dirs, files in os.walk(basedir):
        ...
        for file_name in files:
            ...
            songH5File = hdf5_getters.open_h5_file_read(file_path)
            song = Song(hdf5_getters.get_song_id(songH5File).decode('UTF-8').replace('"', "").replace("/", ""))

            testDanceability = hdf5_getters.get_danceability(songH5File)

            song.artistID = hdf5_getters.get_artist_id(songH5File).decode("UTF-8").replace('"', "").replace("/", "")
            song.albumID = str(hdf5_getters.get_release_7digitalid(songH5File))
            song.albumName = hdf5_getters.get_release(songH5File).decode("UTF-8").replace('"', "").replace("/", "")
            song.artistLatitude = str(hdf5_getters.get_artist_latitude(songH5File))
            song.artistLocation = hdf5_getters.get_artist_location(songH5File).decode("UTF-8").replace('"', "").replace("/", "")
            song.artistLongitude = str(hdf5_getters.get_artist_longitude(songH5File))
            song.artistName = hdf5_getters.get_artist_name(songH5File).decode("UTF-8").replace('"', "").replace("/", "")
            ...
            song.title = hdf5_getters.get_title(songH5File).decode("UTF-8").replace('"', "").replace("/", "")
            song.trackID = hdf5_getters.get_track_id(songH5File).decode("UTF-8").replace('"', "").replace("/", "")
